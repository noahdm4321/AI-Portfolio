...
class Othello(Board):
    ...
    def make_move(self, move=None):
        ...
        print("Attempting to make move:", self.move)
        if self.is_legal_move(self.move):
            print("Move is legal. Executing move.")
            ...
            print("Move is not legal. Move not executed.")
    ...
    def get_winner(self):
        ...
        Returns:
            int: The player index (0 or 1) of the winner. None if it's a tie.
        """
        if self.num_tiles[0] > self.num_tiles[1]:
            return -1
        elif self.num_tiles[0] < self.num_tiles[1]:
    ...
    def is_game_over(self):
        ...
        if self.current_player == 1:
            comp = self.has_legal_move()
            self.current_player = 0
            user = self.has_legal_move()
            self.current_player = 1
        else:
            user = self.has_legal_move()
            self.current_player = 1
        ...
        return not (comp or user) or sum(self.num_tiles) == self.n ** 2
...
def decode_image(contents: Annotated[Any, _atypes.String], channels:int=0, dtype:TV_DecodeImage_dtype=_dtypes.uint8, expand_animations:bool=True, name=None) -> Annotated[Any, TV_DecodeImage_dtype]:
  r"""Function for decode_bmp, decode_gif, decode_jpeg, decode_jxl, decode_webp, and decode_png.
...
TV_DecodeJxl_dtype = TypeVar("TV_DecodeJxl_dtype", bound=_atypes.UInt8)

def decode_jxl(contents: Annotated[Any, _atypes.String], channels:int=0, dtype:TV_DecodeJxl_dtype=_dtypes.uint8, name=None) -> Annotated[Any, TV_DecodeJxl_dtype]:
  r"""Decode a JPEG XL encoded image to a uint8 tensor.
  ...
  *   0: Use the number of channels in the JPEG XL encoded image.
  *   1: output a grayscale image.
  *   3: output an RGB image.
  *   4: output an RGBA image.

  The number of channels must match that of the underlying file.

  Args:
    contents: A `Tensor` of type `string`. 0-D.  The JPEG XL encoded image.
    channels: An optional `int`. Defaults to `0`.
      Number of color channels for the decoded image.
    dtype: An optional `tf.DType` from: `tf.uint8`. Defaults to `tf.uint8`.
    name: A name for the operation (optional).
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx, "DecodeJxl", name, contents, "channels", channels, "dtype",
        dtype)
      return _result
    except _core._NotOkStatusException as e:
      _ops.raise_from_not_ok_status(e, name)
      ...
      pass
    try:
      return decode_jxl_eager_fallback(
  ...
  dtype = _execute.make_type(dtype, "dtype")
  _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "DecodeJxl", contents=contents, channels=channels, dtype=dtype,
                     name=name)
  _result = _outputs[:]
  if _execute.must_record_gradient():
    _attrs = ("channels", _op._get_attr_int("channels"), "dtype",
    ...
    _inputs_flat = _op.inputs
    _execute.record_gradient(
        "DecodeJxl", _inputs_flat, _attrs, _result)
  _result, = _result
  return _result

DecodeJxl = tf_export("raw_ops.DecodeJxl")(_ops.to_raw_op(decode_jxl))


def decode_jxl_eager_fallback(contents: Annotated[Any, _atypes.String], channels: int, dtype: TV_DecodeJxl_dtype, name, ctx) -> Annotated[Any, TV_DecodeJxl_dtype]:
  if channels is None:
    channels = 0
  channels = _execute.make_int(channels, "channels")
  if dtype is None:
  ...
  _result = _execute.execute(b"DecodeJxl", 1, inputs=_inputs_flat,
                             attrs=_attrs, ctx=ctx, name=name)
  if _execute.must_record_gradient():
    _execute.record_gradient(
        "DecodeJxl", _inputs_flat, _attrs, _result)
...
TV_DecodeWebP_dtype = TypeVar("TV_DecodeWebP_dtype", bound=_atypes.UInt8)

def decode_web_p(contents: Annotated[Any, _atypes.String], channels:int=0, dtype:TV_DecodeWebP_dtype=_dtypes.uint8, name=None) -> Annotated[Any, TV_DecodeWebP_dtype]:
  r"""Decode a WebP-encoded image to a uint8 tensor.
  ...
  *   4: output an RGBA image.
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx, "DecodeWebP", name, contents, "channels", channels, "dtype",
        dtype)
      return _result
    except _core._NotOkStatusException as e:
      _ops.raise_from_not_ok_status(e, name)
      ...
      pass
    try:
      return decode_web_p_eager_fallback(
          contents, channels=channels, dtype=dtype, name=name, ctx=_ctx)
    except _core._SymbolicException:
      pass  # Add nodes to the TensorFlow graph.
  # Add nodes to the TensorFlow graph.
  ...
  dtype = _execute.make_type(dtype, "dtype")
  _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "DecodeWebP", contents=contents, channels=channels, dtype=dtype,
                      name=name)
  _result = _outputs[:]
  if _execute.must_record_gradient():
    _attrs = ("channels", _op._get_attr_int("channels"), "dtype",
    ...
    _inputs_flat = _op.inputs
    _execute.record_gradient(
        "DecodeWebP", _inputs_flat, _attrs, _result)
  _result, = _result
  return _result

DecodeWebP = tf_export("raw_ops.DecodeWebP")(_ops.to_raw_op(decode_web_p))


def decode_web_p_eager_fallback(contents: Annotated[Any, _atypes.String], channels: int, dtype: TV_DecodeWebP_dtype, name, ctx) -> Annotated[Any, TV_DecodeWebP_dtype]:
  if channels is None:
    channels = 0
  channels = _execute.make_int(channels, "channels")
  if dtype is None:
  ...
  _inputs_flat = [contents]
  _attrs = ("channels", channels, "dtype", dtype)
  _result = _execute.execute(b"DecodeWebP", 1, inputs=_inputs_flat,
                             attrs=_attrs, ctx=ctx, name=name)
  if _execute.must_record_gradient():
    _execute.record_gradient(
        "DecodeWebP", _inputs_flat, _attrs, _result)

...
/// * expand_animations: Controls the output shape of the returned op. If True, the returned op
...
///
/// Returns:
/// * `Output`: The image tensor.
class DecodeJxl {
 public:
  /// Optional attribute setters for DecodeJxl
  struct Attrs {
    /// Defaults to 0
    TF_MUST_USE_RESULT Attrs Channels(int64 x) {
      Attrs ret = *this;
  ...
  DecodeJxl(const ::tensorflow::Scope& scope, ::tensorflow::Input contents);
  DecodeJxl(const ::tensorflow::Scope& scope, ::tensorflow::Input contents, const
          DecodeJxl::Attrs& attrs);
  operator ::tensorflow::Output() const { return image; }
  operator ::tensorflow::Input() const { return image; }
  ::tensorflow::Node* node() const { return image.node(); }
...
///
/// Returns:
/// * `Output`: 4-D with shape `[num_frames, height, width, channels]`.
class DecodeWebP {
 public:
  /// Optional attribute setters for DecodeWebP
  struct Attrs {
    /// Number of color channels for the decoded image.
    ///
    /// Defaults to 0
  ...
  DecodeWebP(const ::tensorflow::Scope& scope, ::tensorflow::Input contents);
  DecodeWebP(const ::tensorflow::Scope& scope, ::tensorflow::Input contents,
           const DecodeWebP::Attrs& attrs);
  operator ::tensorflow::Output() const { return image; }
  operator ::tensorflow::Input() const { return image; }
  ::tensorflow::Node* node() const { return image.node(); }
